...
The current data is generated with Ornith as the sole orchestrator plus the supergemma
leaf (full steps in [`README.md`](README.md)):

```bash
# serve the two local models (see environment.txt for exact builds)
./scripts/local-serving/serve-models.sh start                          # leaf  :8771
PARALLEL=1 CTX=65536 NOTHINK=1 ./scripts/local-serving/serve-ornith.sh  # orchestrator :8774
